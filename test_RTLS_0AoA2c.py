...
from solver.landing.solver_jit import SuicideBurnSolver
...
last_landing_point = np.zeros(3)
while True: 
    position = vessel.position()
    velocity = vessel.velocity()
    body_ip1 = SSS.predict_impact_point()[0][0, :3]
    body_ip2 = IPS.predict_impact_point()['event_X'][0, :3]
    # ratio = position[0] / atd
    targ_ip1 = transform_to_target_frame(body_ip1, target_origion, tgt[0], tgt[1])
    targ_ip2 = transform_to_target_frame(body_ip2, target_origion, tgt[0], tgt[1])
    estimated_landing_point = 0.5*(targ_ip1 + targ_ip2)
    horizontal_error = norm(estimated_landing_point[1:3])
    ...
    vessel.vessel.control.throttle = 0
    igh = targ_ip1[0]
    print(igh, horizontal_error)
    SSS.mass = IPS.mass = vessel.vessel.mass
    SSS.position = IPS.position = transform_to_body_frame(position, target_origion, tgt[0], tgt[1])
    SSS.velocity = IPS.velocity = transform_to_body_frame(velocity, target_origion, tgt[0], tgt[1], is_velocity=True)
    SSS.drag_coefficient = IPS.drag_coefficient = vessel.vessel.flight(brf).drag_coefficient
...
while True:
    ...
    targ_ip = transform_to_target_frame(body_ip, target_origion, tgt[0], tgt[1])
    estimated_landing_point = targ_ip
    target_direction = - normalize(velocity)
    vessel.update_ap(target_direction)
