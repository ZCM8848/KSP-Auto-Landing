...
def air_vector(r,rotation_speed_in_radians):
  ...
  v_air = array( [x*cos(-a) + z*sin(-a),y,-x*sin(-a) + z*cos(-a)] ) - r
  return v_air
...
def simulate(r0,v0,dt=1.0,Cd=0.6,A=1,mass=1000,
  ...
  while cur_radius > body_radius and (t-t0) < max_t:
    ...
    # if t - t0 > max_t:
    #   print("t > %.1f. Taking too long initial_radius=%.1f final_radius=%.1f" % (max_t,norm(r0),norm(r)-body_radius))
...
def save(trajectory,filename):
  f = open(filename,'w')
  print("time x y z vx vy vz ax ay az")
  for t,r,v,a in trajectory:
    print(f"{t}, {r[0]}, {r[1]}, {r[2]}, {v[0]}, {v[1]}, {v[2]}, {a[0]}, {a[1]}, {a[2]}")
  f.close()

if __name__ == "__main__":
  r = array([37436.,883.,21480.])
  v = array([674.,-0.5,811])
  traj = simulate(r,v,body_radius=6000,max_t=60000)
  print(traj[-1])
  # save(traj,".\\traj.dat")