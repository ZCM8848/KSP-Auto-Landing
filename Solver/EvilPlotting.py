...
#    print('s',s.shape)
#    print('z',z.shape)
    r1=v_data['max_thrust'] * v_data['min_throttle']
    r2=v_data['max_thrust'] * v_data['max_throttle']
    ...
    X, Y, Z = R*np.cos(P), R*np.sin(P), R*(np.tan(v_data['glide_slope_cone']))
    ...
    plt.plot(x,0*x+np.degrees(v_data['thrust_pointing_constraint']))
    plt.plot(t,u_angle)
    plt.title('Thrust angle')
    
    alpha=1 / 9.80665 / v_data['specific_impulse']
    z0_term = (v_data['mass'] - alpha * r2)  # see ref [2], eq 34,35,36
    z1_term = (v_data['mass'] - alpha * r1)
    lim=[]
    lim2=[]
    n=0
    z=z.flatten()
