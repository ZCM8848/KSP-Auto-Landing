import _gfold_solver_p3 as _intf
import numpy

def cg_solve(x0,sparse_params,g_vec,z0_term_log,z0_term_inv,):
    ...
    if z0_term_log.shape != (1, 160):
        raise TypeError("Parameter z0_term_log should have shape (1, 160), but has shape %s" % str(z0_term_log.shape))
    z0_term_inv = numpy.ascontiguousarray(numpy.asmatrix(z0_term_inv), dtype=float)
    if z0_term_inv.shape != (1, 160):
        raise TypeError("Parameter z0_term_inv should have shape (1, 160), but has shape %s" % str(z0_term_inv.shape))

    vars_dict, stats_dict = _intf.solve(x0,sparse_params,g_vec,z0_term_log,z0_term_inv,)
