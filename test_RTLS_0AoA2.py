...
while True: 
    ...
    velocity = vessel.velocity()
    try:
        body_ip = SSS.predict_impact_point()[0][0, :3]
    except:
        body_ip = last_ip
    last_ip = body_ip
    # ratio = position[0] / atd
    targ_ip = transform_to_target_frame(body_ip, target_origion, tgt[0], tgt[1])
    last_landing_point = estimated_landing_point
    estimated_landing_point = targ_ip
    dt = space_center.ut - last_ut
    landing_point_velocity = (estimated_landing_point - last_landing_point) / dt
    altitude = vessel.vessel.flight(trf).surface_altitude
    horizontal_error = norm(estimated_landing_point[1:3])
    gfold_start_altitude = max(5 * horizontal_error, 3000)

    target_direction = -velocity + array([0, estimated_landing_point[1], estimated_landing_point[2]]) - landing_point_velocity
    ...
    igh = targ_ip[0]
    print(targ_ip)
    SSS.mass = vessel.vessel.mass
    SSS.position = transform_to_body_frame(position, target_origion, tgt[0], tgt[1])
    SSS.velocity = transform_to_body_frame(velocity, target_origion, tgt[0], tgt[1], is_velocity=True)
    SSS.drag_coefficient = vessel.vessel.flight(brf).drag_coefficient
    if igh <= 0:
        vessel.vessel.control.throttle = 1
        break
    last_ut = space_center.ut
...
while True:
    ...
    body_ip = SSS.predict_impact_point()[0][0, :3]
    targ_ip = transform_to_target_frame(body_ip, target_origion, tgt[0], tgt[1])
    vessel.vessel.control.throttle = throttle_pid.update(-targ_ip[0], space_center.ut - last_ut)
