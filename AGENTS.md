...
- `src/recovery/ksp/` is the **only** place that imports `krpc`. All KSP I/O goes through this layer (`ConnectionManager`, `VesselHandle`, `VesselControls`, `FlightState`). Guidance/control/data must stay KSP-free.
- `src/recovery/guidance/` — impact predictor (`LandingPredictor`, scipy RK45 + numba RK4 fast path), aero models (`DragModel` offline, `KrpcAeroModel` per-step RPC).
...
- `VesselHandle.init_predictor()` makes a one-time ~25 ms RPC to build an offline `DragModel`; `predict()` after that is pure local and safe at control-loop rates.
...
- `recovery.guidance.{predictor,_numba,aerodynamics}` are also mypy-ignored.
- The `scipy.*` mypy override is listed twice in pyproject (harmless duplicate).
