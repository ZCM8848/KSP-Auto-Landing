...
import pyautogui
from RTLS_solver import ImapctPointSolver
...
ips = ImapctPointSolver(vessel.vessel.mass,
                        vessel.vessel.position(brf),
                        vessel.vessel.velocity(brf),
                        body.gravitational_parameter,
                        body.angular_velocity(bnrf),
                        space_center.transform_position((0, 0, 0), trf, brf))
error = [np.inf]
while True:
    ...
    ips.position = vessel.vessel.position(brf)
    ips.velocity = vessel.vessel.velocity(brf)
