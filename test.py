...
target_position = array([3000,400,150])
...
while True:
    ...
    throttle = 0.5*(velocity_error*0.3 + position_error*0.1)[0]
    throttle = clamp(throttle,0.2,1)
    #throttle = norm(target_direction) / available_thrust
...
while vessel.velocity(target_reference_frame)[0] >= -10:
